...
sap.ui.define([
		...
		return BaseController.extend("carrpasp.controller.SegvalTbl", {
			...
			onInit : function () {
				...
				this.getRouter().getRoute("segvalTbl").attachPatternMatched(this._onObjectMatched, this);
			...
			onListUpdateFinished : function (oEvent) {
				...
				if (this.byId("lineItemsList").getBinding("items").isLengthFinal()) {
					if (iTotalItems) {
						...
						sTitle = this.getResourceBundle().getText("detailLineItemTableHeadingCount", [0]);
					}
			...
			onFilter: function() {
			    ...
			    this.getView().byId("lineItemsList").getBinding("items").filter(aFilters);
			},  
			...
			this.oCurContext = this.getModel().createEntry("/SEGVAL", {
				properties: oProperties
			});
	      	dialog.setBindingContext(this.oCurContext);
	      	dialog.setTitle("Добавление значения сегмента");
	      	dialog.open();
			},
			...
			onUpd : function(event) {
		      ...
		      if (this.oCurContext) {
		      	dialog.setBindingContext(this.oCurContext);
		      	dialog.setTitle("Редактирование значения сегмента");
		      	dialog.open();
		      }
			},
